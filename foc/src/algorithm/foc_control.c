...
static float FOC_CoggingCompLookupIq(float mech_angle_rad, float speed_ref_rad_s)
{
    float angle_rad;
    float index_f;
    uint16_t index_0;
    ...
    if ((g_cogging_comp_status.enabled == 0U) ||
        (g_cogging_comp_status.available == 0U) ||
        (g_cogging_comp_status.point_count < 2U))
    {
        return 0.0f;
    }

    angle_rad = Math_WrapRad(mech_angle_rad);
    index_f = angle_rad * ((float)g_cogging_comp_status.point_count / FOC_MATH_TWO_PI);
    index_0 = (uint16_t)index_f;
    if (index_0 >= g_cogging_comp_status.point_count)
    {
        index_0 = (uint16_t)(g_cogging_comp_status.point_count - 1U);
    }
    index_1 = (uint16_t)((index_0 + 1U) % g_cogging_comp_status.point_count);
    frac = index_f - (float)index_0;

    iq_0 = (float)g_cogging_comp_table_q15[index_0] * g_cogging_comp_status.iq_lsb_a;
    iq_1 = (float)g_cogging_comp_table_q15[index_1] * g_cogging_comp_status.iq_lsb_a;
    iq_comp = iq_0 + (iq_1 - iq_0) * frac;

    speed_abs = fabsf(speed_ref_rad_s);
    if (g_cogging_comp_status.speed_gate_rad_s > 1e-6f)
    {
        if (speed_abs >= g_cogging_comp_status.speed_gate_rad_s)
        {
            return 0.0f;
        }

        iq_comp *= (1.0f - (speed_abs / g_cogging_comp_status.speed_gate_rad_s));
    }

    return Math_ClampFloat(iq_comp,
                           -g_cogging_comp_status.iq_limit_a,
                           g_cogging_comp_status.iq_limit_a);
}
...
static float FOC_CurrentSoftSwitchUpdateBlend(float target_blend, float dt_sec)
{
    float alpha;

    target_blend = Math_ClampFloat(target_blend, 0.0f, 1.0f);
    if (g_current_soft_switch_blend_initialized == 0U)
    {
        g_current_soft_switch_status.blend_factor = target_blend;
        g_current_soft_switch_blend_initialized = 1U;
        return g_current_soft_switch_status.blend_factor;
    }

    alpha = dt_sec / (FOC_CURRENT_SOFT_SWITCH_BLEND_TAU_DEFAULT_SEC + dt_sec);
    alpha = Math_ClampFloat(alpha, 0.0f, 1.0f);

    g_current_soft_switch_status.blend_factor += (target_blend - g_current_soft_switch_status.blend_factor) * alpha;
    g_current_soft_switch_status.blend_factor = Math_ClampFloat(g_current_soft_switch_status.blend_factor, 0.0f, 1.0f);
    return g_current_soft_switch_status.blend_factor;
}
                                             ...
                                             float dt_sec)
{
    ...
    blend_factor = FOC_CurrentSoftSwitchUpdateBlend(target_blend, dt_sec);
                            ...
                            float dt_sec)
{
    ...
    motor->iq_target = FOC_PIDRunCore(speed_pid,
                                      ...
                                      0.0f,
                                      dt_sec);
    motor->iq_target += FOC_CoggingCompLookupIq(sensor->mech_angle_rad.output_value,
                                                 speed_ref_rad_s);
