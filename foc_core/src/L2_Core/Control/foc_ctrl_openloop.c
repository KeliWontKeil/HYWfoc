...
void FOC_OpenLoop_RunStep(foc_motor_t *motor, float dt_sec)
{
    ...
    motor->ctrl.iq_target = (target_elec >= 0.0f) ? FOC_OPENLOOP_CURRENT_A : -FOC_OPENLOOP_CURRENT_A;

    /* 对称双向斜坡逼近目标速度 */
    virtual_speed = motor->openloop_state.virtual_speed_rad_s;
    {
        float speed_step = motor->openloop_state.ramp_rate_rad_s2 * dt_sec;
    ...
    motor->outer_loop.ramped_speed_rad_s = motor->openloop_state.mech_speed_rad_s;
}
