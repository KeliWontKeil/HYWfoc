...
static inline float FOC_FilterGate_IqLpf(
    FOC_FILTER_TYPEDEF(FOC_FILTER_CURRENT_LOOP_IQ) *f, float input)
{
#if (FOC_FILTER_CURRENT_LOOP_IQ == FOC_FILTER_TYPE_LPF1)
    return FOC_FilterMath_Lpf1Step(f, input, FOC_FILTER_CURRENT_LOOP_IQ_LPF_ALPHA);
#elif (FOC_FILTER_CURRENT_LOOP_IQ == FOC_FILTER_TYPE_KALMAN)
    return FOC_FilterMath_KalmanStep(f, input);
#else
    (void)f;
    return input;
