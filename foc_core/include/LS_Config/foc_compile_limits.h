...
#endif
#if ((FOC_CURRENT_LOOP_ISR_MODE != FOC_ISR_MODE_2ISR) && (FOC_CURRENT_LOOP_ISR_MODE != FOC_ISR_MODE_3ISR))
...
#define DEBUG_STREAM_OSC_DEFAULT_PARAM_MASK ( \
    ...
    ((DEBUG_STREAM_OSC_DEFAULT_SHOW_ANGLE_ACTIVE_ELEC != FOC_CFG_DISABLE) ? DEBUG_STREAM_OSC_PARAM_ANGLE_ACTIVE_ELEC : 0U) | \
    ((DEBUG_STREAM_OSC_DEFAULT_SHOW_ANGLE_STANDBY_ELEC != FOC_CFG_DISABLE) ? DEBUG_STREAM_OSC_PARAM_ANGLE_STANDBY_ELEC : 0U) | \
    ((DEBUG_STREAM_OSC_DEFAULT_SHOW_PHASE_LAG != FOC_CFG_DISABLE) ? DEBUG_STREAM_OSC_PARAM_PHASE_LAG : 0U))
...
/* 速域切换滞回约束：升域进入门槛(high_th*SCALE)必须高于降级门槛(low_th)，防止反滞回振荡。
 * 门限为浮点宏，AC5 预处理器不支持浮点 #if，由 FOC_SourceMgr_Init 运行时校验（非法时禁用切换）。 */
...
#define FOC_CONTROL_SRC_IS_ANGLE_CAPABLE(src) \
    ...
    /* FULL 构建可在运行时切到 SPEED_ANGLE；角度控制依赖编码器可靠绝对角度，
     * 运行时 Select 会锁定 LOW 源不作速域切换，故 LOW 必须 angle-capable。
     * HIGH 可为 SMO（仅服务速度模式的高速无感段）。 */
    #if !FOC_CONTROL_SRC_IS_ANGLE_CAPABLE(FOC_CONTROL_LOW_SOURCE)
    #error "FULL build (runtime switchable to SPEED_ANGLE) requires ENCODER or HFI as low-speed source"
    #endif
#endif
