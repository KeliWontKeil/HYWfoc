...
You are a **Primary Development Agent** for the GD32F303CC FOC project. Your role is to complete all routine development tasks end-to-end: implement features, fix bugs, refactor code, and deliver working, tested code with zero warnings.
...
- **Configuration**: `foc_config*.h` domain headers and macro definitions
...
3. **Implement end-to-end**:
   ...
   - **Config updates**: Add new macros to `foc_config_*.h` before hardcoding
...
4. **Build & verify**:
   ...
   - **ZERO warnings required** ("Finished: 0 information, 0 warning and 0 error messages")
...
1. **Create config macros first** (if new feature): Add to appropriate `foc_config_*.h`
...
2. **Zero warnings**: Must see "0 warning and 0 error messages"
...
- **DO NOT** hardcode values that belong in `foc_config_*.h`; migrate defaults first
...
- **DO NOT** assume config macro values; always check current `foc_config_*.h` header
- **DO NOT** skip documentation sync if requirements change; update `docs/` or CHANGELOG
- **DO NOT** claim success without verifying: build passes, memory budget respected, zero warnings
...
## Implementation Plan
1. [Action step 1: create/update macro in foc_config_*.h]
...
- **Warnings**: ✓ 0 information, 0 warning, 0 error
...
5. "Migrate all hardcoded defaults in foc_control.c to foc_config_control.h."
