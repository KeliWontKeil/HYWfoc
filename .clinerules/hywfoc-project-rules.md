...
- `docs/` — 库级文档
  ...
  - `docs/protocol-parameters.md` — 协议参数
- `docs/engineering/dev-guidelines/rules/cn/` — 仓库规则文件（中文）
- `.github/` — 工作流治理

## Layer architecture

依赖方向严格单向：`LS` → `L1` → `L2` → `L3` → `L4`
...
- 语义化版本 `MAJOR.MINOR.PATCH`（当前 `v2.0.5`）
...
4. `AI_INITIALIZATION.md` — AI 初始化指引
5. `CHANGELOG.md` — 变更记录
...
- 如果认为prompt有什么地方说不通，请不要使用自认为的答案然后继续修改，而是停下向我确认。
...
#### 架构扩展时已有路径零回归
- 新增功能模块通过条件编译宏完全裁剪，未选中的模块不进入编译
- 扩展每一步后必须编译验证，已有路径（如"有感"）功能不得退化
- 宏裁剪链路必须声明/定义/调用三者一致

#### 构建入口同步
- 新增/重命名/删除源文件时，必须同步更新所有构建入口：`builder.params.sourceList` + `.eide/eide.yml` 的 FOC_CORE 列表
- 文件重命名时，同步更新所有 `#include` 引用点、构建入口和 EIDE 排除列表，避免遗漏导致的链接错误

#### 头文件与源文件命名一致性
- 每个 `.c` 文件应有对应的独立 `.h` 文件（即使仅做聚合转发）
- `.c` 与 `.h` 文件名必须一致，重命名时两者必须同步

#### 数据结构扩展原则
- 新增条件编译子结构体放在 `foc_motor_t` 现有条件编译段之后、非条件编译段之前
- 不改变已有字段的声明顺序，确保内存布局兼容

#### 传感器硬件与算法解耦
- 硬件存在性（如编码器是否焊接）与算法使用（是否作为反馈源）通过不同层次的宏独立控制
- 有效性检查位置必须收口为单一检查点，避免多处重复检查导致逻辑不一致

#### 调试数据源
- 调试/遥测输出应反映控制算法实际看到的数据（如 `ctrl_input`），而非独立采集原始传感器数据
