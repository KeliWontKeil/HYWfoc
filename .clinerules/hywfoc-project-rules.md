...
| `L1` | `foc/src/L1_Orchestration/foc_app.c` | 启动流程、控制入口、数据流协调 |
| `L2` | `foc/src/L2/` | 按功能分3块：Control/Protocol/Runtime |
| `L2/Control` | `foc/src/L2/Control/foc_ctrl_*.c` | 控制算法、传感器采样、SVPWM执行 |
| `L2/Protocol` | `foc/src/L2/Protocol/foc_protocol_*.c` | 协议帧解析、参数通道 |
| `L2/Runtime` | `foc/src/L2/Runtime/foc_runtime_*.c` | 运行时状态机、调度、输出适配 |
| `L3` | `foc/src/L3/foc_math_*.c, foc_pal_*.c` | 数学与 LUT + 平台抽象API（合并） |
...
运行时链：`foc_runtime_entry → foc_protocol_entry → foc_runtime_fsm → foc_runtime_store → foc_runtime_output`

保持单向依赖，禁止跨层逆向调用。L2 各块间通过 L1 统一传递数据，不直接跨块调用。
