...
HYW FOC 是一个可裁剪、结构清晰、可移植、扩展性强、支持有感+无感的单电机 FOC 可复用库。
- 集成有感/无感FOC算法无刷电机驱动
- 集成指令控制系统和多种保护机制
...
- 探索FOC算法实现方式，优化相关算法
- 学习"平台适配接口 + 控制核心分层解耦"的开发方式（不属于任何标准，作者自己摸索出来的，内部还是有点乱，不想再重构了，只是借鉴了相关思想，请勿将其作为某种标准化的开发方式）。
- 探索 AI 辅助嵌入式开发的新工作流。
...
- 电流环开环/闭环软切换抑制小电流下噪声
- 齿槽转矩补偿（在线标定 + 运行时补偿）
- 完整的控制和监控协议，支持多种命令和自定义格式化输出/反馈回报
- 错误检测及安全机制
...
| 指令化查询/控制 | ✅ 可用 |
| 无感 FOC | ✅ 可用 |
| 多电机驱动 | ❌ 不支持，以后可能也不会支持（似乎后有支持的可能性？） |
...
- 其他协议解析，指令轮询，命令触发等按照对ISR阻塞程度分别编排在中断和主循环中
- 具体去问AI或是看架构文档

> 现阶段：有感/无感 FOC框架，持续进行控制效果优化、代码优化、BUG修复。
> 目前已进入稳定版本
> 后续计划：高频注入无感 FOC
...
- 通过平台 API（L3_Hal/foc_platform_api.h）连接具体的外设相关驱动和实现。
...
很多AI开发的完全不能细看，看起来还行细看一坨，目前结构已经完全重构，应该很清晰了
...
下版本目标：v2.3.0（控制效果与算法持续优化）：
- 控制效果优化、PID 参数按速度/负载自适应
- 有感/无感控制进一步打磨
（v2.2.0 已完成：无感控制结构性问题修复——速度环重构为纯速度环、启动假速度修复等，详见 CHANGELOG）

极长期目标（不知道会拖到什么时候去了）：
1. 完成无感FOC开发
2. 开发一个图形化参数修改上位机
3. ~~把这个库开发成世界上最牛逼的FOC库~~（做梦中）
